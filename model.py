# adapted from: https://huggingface.co/rasbt/llama-3.2-from-scratch/blob/main/model.py
...
class Llama3MoE(nn.Module):
    ...
    def forward(self, in_idx, expert_map):
        ...
        for block in self.layers:
            x, block_expert_loss = block(x, mask, self.cos, self.sin, expert_map)
            expert_loss += (block_expert_loss * loss_mask).sum() / loss_mask.sum().clamp_min(1)

        x = self.norm(x)
        logits = self.lm_head(x.to(self.cfg["dtype"]))
        return logits, expert_loss / self.cfg['n_layers']
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask, cos, sin, expert_map):
        ...
        x, expert_loss = self.moe(x, expert_map)
        x = x + shortcut  # Add the original input back
...
class MoE(nn.Module):
    ...
    def forward(self, x, expert_map, eps: float = 1e-9):
        ...
        r = torch.softmax(logits, dim=-1)  # (b, s, e)

        topk_vals, topk_idx = logits.topk(self.k, dim=-1)   # (b, s, k)
        ...
        outs = [expert(x) for expert in self.experts]      # list of (b, s, emb_dim)
        outs = torch.stack(outs, dim=-1)                   # (b, s, emb_dim, e)
        out = (outs * r_topk.unsqueeze(-2)).sum(dim=-1) 

        eps = 1e-9
        allowed = expert_map[:, None, :].to(r.dtype)          # (b, 1, e)
        mass = (r * allowed).sum(dim=-1).clamp_min(eps)       # (b, s)
        expert_loss = - mass.log()                            # (b, s)
